...
// Per-tensor requantization: 32-bit accumulator -> 32-bit result
inline vint32m4_t RequantizeVectorPerTensorS32(
    vint32m4_t v_acc,
    const int32_t multiplier,
    const int effective_right_shift,
    const int32_t output_offset,
    const int32_t activation_min,
    const int32_t activation_max,
    const size_t vl) {

  // Rounding constant for 64-bit shift
  ...
  // 32x32 -> 64-bit product (hi / lo parts)
  ...
  // Add 64-bit rounding value using 32-bit ops + carry
  ...
  if (effective_right_shift == 0) {
    ...
    v_res32 = __riscv_vreinterpret_v_u32m4_i32m4(
        __riscv_vor_vv_u32m4(
            v_lo_usrl,
            __riscv_vreinterpret_v_i32m4_u32m4(v_hi_sll),
            vl));
  } else {
...
// Per-channel requantization: 32-bit accumulator -> 32-bit result
inline vint32m4_t RequantizeVectorPerChannelS32(
    vint32m4_t v_acc,
    vint32m4_t v_multiplier,
    vint32m4_t v_shift,
    const int32_t output_offset,
    const int32_t activation_min,
    const int32_t activation_max,
  ...
  // effective_shift = 31 - shift (TFLM convention)
  ...
  {
    // rounding = 1ll << (effective_shift - 1)
    ...
    vbool8_t v_mask_round_ge_32 =
        __riscv_vmandn_mm_b8(v_mask_right_shift, v_mask_round_lt_32, vl);
    ...
    vbool8_t v_mask_shift_ge_32 =
        __riscv_vmandn_mm_b8(v_mask_right_shift, v_mask_shift_lt_32, vl);
  ...
  // --- Left-shift path (effective_shift <= 0) ---
  vint32m4_t v_res_left;
  {
    ...
    v_res_left = __riscv_vsll_vv_i32m4_m(
        ...
        __riscv_vreinterpret_v_i32m4_u32m4(v_left_shift_amount),
        vl);
  }
